...
<!--  검색 시작  --><form action="programAdminList.do" method="post" name="userinput">
...
%>
	...
	%>
			...
			<td >
				<div class="container-fluid">
					<div class="row">
						<div class="col-md-12">
						<img alt="<%=programAdminList.get(i).getImage()%>" title="<%=programAdminList.get(i).getImage()%>" src=upload/<%=programAdminList.get(i).getImage()%> width='100' height='80'  />
						</div>
					</div>
				</div> 
				                                                   
				<td align="center"><a href="programDetail.do?p_num=<%=programAdminList.get(i).getP_num()%>"><%=programAdminList.get(i).getP_num()%></a></td>

				<td align="center">
					<%if(programAdminList.get(i).getP_num()!=0){ %> <%for(int a=0;a<=programAdminList.get(i).getP_num()*2;a++){ %>
					 <%} }else{ } %><a href="programDetail.do?p_num=<%=programAdminList.get(i).getP_num()%>"> 
				...
				<td align="center"><a href="programDetail.do?p_num=<%=programAdminList.get(i).getP_num()%>"><%=programAdminList.get(i).getP_name() %></a></td>
				
				<td align="center"><a href="programDetail.do?p_num=<%=programAdminList.get(i).getP_num()%>"><%=programAdminList.get(i).getStartdate() %></a></td>
				<td align="center"><a href="programDetail.do?p_num=<%=programAdminList.get(i).getP_num()%>"><%=programAdminList.get(i).getCount() %></a></td>
				<td align="center"><a href="programDetail.do?p_num=<%=programAdminList.get(i).getP_num()%>"><%=programAdminList.get(i).getTotal_number() %></td>
	...
	<section id="pageList">
		...
		&nbsp;
		<%}else if (nowPage>5){ %>
		<a href="programSearch.do?category=<%=s_category %>&item=<%=s_item %>&searchText=<%=s_searchText %>&page
=<%=endPage-5%>">[이전]</a>&nbsp;
		<%} %>
