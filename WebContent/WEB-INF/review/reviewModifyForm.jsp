...
window.onload = function selectcategory(){
	  var length = document.getElementById("P_name").options.length;
	  for(i = 0 ; i < length ; i ++){
		  if(document.getElementById("P_name").options[i].value == "<%=review.getP_name()%>") 
		  {
				document.getElementById("P_name").options[i].selected = true;
				break;
			}
		}
	}
...
<br>
    ...
    <div class="col-md-8">
		...
		<form action="reviewModifyPro.do" method="post" name="modifyform">
		
			<input type="hidden" name="Rev_num" value="<%=review.getRev_num()%>" />
			...
			<table class="table table-striped">
				...
				<tr>
					<td class="td_left"><label for="M_id">글쓴이</label></td>
					<td class="td_right">
					<input name="M_id" value="<%=review.getM_id()%>" type="hidden">
					<input type="text" name="M_id" id="M_id"
						value="<%=review.getM_id()%>" disabled="disabled" /></td>
				</tr>
				...
				<tr>
                	...
                	<td><input type="text"  class="form-control" name="Title" type="text"
						id="Title" required="required" value="<%=review.getTitle()%>"/></td>
            	</tr>
				...
				<tr>
                	<td>글내용</td>
                	<td><textarea rows="10" cols="50" name="Rev_content"  class="form-control" ><%=review.getRev_content()%></textarea></td>
            	</tr>
